...
class AsyncCopyTest(TestCase, jtu.CudaArchSpecificTest):
  ...
  def test_tma_scatter_tiled(self, swizzle, shape, dtype, transpose_tiles):
    if not jtu.is_cuda_compute_capability_at_least("10.0"):
      self.skipTest(
    ...
    np.testing.assert_array_equal(y[idx, slice(col_slice, 2 * col_slice)], x)
